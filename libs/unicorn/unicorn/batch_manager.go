...
type BatchManager struct {
	parent     krunloop.EventPoster[*Unicorn]
	maxDelayMs int
	name       string
	isInFlight bool // true means the event is already being scheduled
	reasons    []string
	fn         func(context.Context, *Unicorn)
}

func NewBatchManager(parent krunloop.EventPoster[*Unicorn], maxDelayMs int, name string, fn func(context.Context, *Unicorn)) *BatchManager {
	return &BatchManager{
		parent:     parent,
...
func (bm *BatchManager) TrySchedule(ctx context.Context, reasons ...string) {
	krunloop.VisitResource(bm.parent, func(ss *Unicorn) {
...
func (bm *BatchManager) TryScheduleInternal(ctx context.Context, reasons ...string) {
	...
	slog.InfoContext(ctx, "batch manager in-bound",
		slog.String("event", "BatchManager.InBound"),
		slog.String("name", bm.name),
		slog.String("result", scheduled),
...
func (bpe *BatchProcessEvent) Process(_ context.Context, ss *Unicorn) {
	var reasons []string
	bpe.parent.isInFlight = false
	reasons = bpe.parent.reasons
	bpe.parent.reasons = nil

	slog.InfoContext(bpe.ctx, "batch manager out-bound",
		slog.String("event", "BatchManager.OutBound"),
		slog.String("name", bpe.parent.name),
		slog.Any("reason", reasons))
