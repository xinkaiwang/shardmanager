...
func TestAssembleUnassignSolver(t *testing.T) {
	...
	fn := func() {
		...
		// Step 5: remove shard_1 (unassign solver should be triggered)
		klogging.Info(ctx).Log("Step5", "remove shard_1")
...

		{
			...
			assert.Equal(t, true, waitSucc, "应该能在超时前接受提议, 耗时=%dms", elapsedMs)
		}

		{
			waitSucc, elapsedMs := setup.WaitUntilPilotNode(t, workerFullId, func(pnj *cougarjson.PilotNodeJson) (bool, string) {
				...
				if len(pnj.Assignments) > 0 {
					return false, "polot 有 assignment"
				}
				return true, "没有 assignment"
			}, 30*1000, 1000)
			assert.Equal(t, true, waitSucc, "应该能在超时前 pilotNode update, 耗时=%dms", elapsedMs)

			// Step 6: simulate eph node update
