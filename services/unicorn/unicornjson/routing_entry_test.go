...
func TestWorkerEntryJsonMarshal(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: `{"worker_id":"unicorn-worker-75fffc88f9-fkbcm","addr_port":"10.0.0.32:8080","capacity":10,"assignments":[{"shd":"shard-1","idx":1,"asg":"asg-1"},{"shd":"shard-2","asg":"asg-2"}]}`,
		},
		{
			name: "最小字段",
			input: NewWorkerEntryJson(
				"unicorn-worker-75fffc88f9-xj9n2",
				"10.0.0.33:8080",
				20,
			),
			expected: `{"worker_id":"unicorn-worker-75fffc88f9-xj9n2","addr_port":"10.0.0.33:8080","capacity":20}`,
		},
	}
