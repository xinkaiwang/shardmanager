...
| 2026-08-09 | KLOG-009 | **改判 WONTFIX（待王总追认）**：`StopAndWaitForExit()` 是横贯 shardmgr 的接口模式（10+ 实现方，solver_group.go:36 为接口方法），为两行生命周期日志改公共接口签名，代价远超 P3 收益 | 实施时发现，偏离原计划故显式记录 |
